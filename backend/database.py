...
class Database:
    ...
    def insert_idea(self, title, message, user_id, attachment=None):
        if attachment:
            return self._execute("""
    ...
    def add_comment(self, user_id, msg_id, body):
        return self._execute("""
    ...
    def edit_comment(self, comment_id, user_id, new_body):
        self._execute("""
            UPDATE public.comments SET body = %s WHERE id = %s AND user_id = %s;
    ...
    def apply_vote(self, user_id, msg_id, direction):
        ...
        if direction == "up":
            self._execute("INSERT INTO public.userupvotes (user_id, msg_id) VALUES (%s, %s);", (user_id, msg_id), commit=True)
        elif direction == "down":
            self._execute("INSERT INTO public.userdownvotes (user_id, msg_id) VALUES (%s, %s);", (user_id, msg_id), commit=True)

        upvotes = self._execute("SELECT COUNT(*) FROM public.userupvotes WHERE msg_id = %s;", (msg_id,), fetchone=True)["count"]
        downvotes = self._execute("SELECT COUNT(*) FROM public.userdownvotes WHERE msg_id = %s;", (msg_id,), fetchone=True)["count"]
